//! Reference conformance runner: reads the language-neutral corpus and checks it
//! against the Rust `colla` implementation.
//!
//! See `conformance/README.md` for the fixture format. Unknown fixture kinds are
//! a hard error so the runner can never silently skip a case. RichText content is
//! not part of the current seed corpus and its neutral conversion is deferred.
...
/// Absolute path to `conformance/corpus`, resolved from this crate's location.
pub fn corpus_dir() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../../corpus")
}

/// Every `*.json` fixture under `root`, sorted for deterministic ordering.
pub fn fixture_files(root: &Path) -> Vec<PathBuf> {
...
/// Runs one fixture, dispatching by `kind`. Returns `Err` on a conformance
/// failure and panics on a malformed fixture (unknown kind, missing field).
pub fn run_fixture(id: &str, fixture: &Json) -> Result<(), String> {
    let kind = fixture
        .get("kind")
        .and_then(Json::as_str)
        .unwrap_or_else(|| panic!("{id}: missing `kind`"));
    match kind {
...
pub fn value_from_neutral(json: &Json) -> Result<Value, String> {
...
pub fn change_from_neutral(json: &Json) -> Result<Change, String> {

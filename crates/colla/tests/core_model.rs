use colla::{
    ...
    TieBreak, Utf16PositionError, Value,
};
...
fn builder_rich_text_treats_embeds_as_atomic_units() {
    ...
    assert_eq!(base.as_rich_text().unwrap().span_count(), 3);
    let value_golden = [
    ...
    assert_eq!(
        rich.iter_spans()
            .filter(|span| matches!(span.content(), colla::RichContent::Embed(_)))
            .count(),
        2
    );
}
...
fn algebra_bytes_match_javascript_golden_across_all_change_kinds() {
    let base = Value::map([
        ...
        (
            "rich",
            Value::rich_text(
                RichText::from_spans(vec![RichSpan::text("ab", Attrs::new())]).unwrap(),
            ),
        ),
...
fn rich_text_change_checked_constructor_rejects_length_overflow() {
    let result = RichTextChange::try_new(vec![
        RichTextOp::Retain {
            len: usize::MAX,
            attrs: AttrPatch::new(),
        },
    ...
    assert_eq!(result, Err(colla::ValueError::SequenceLengthOverflow));
}
